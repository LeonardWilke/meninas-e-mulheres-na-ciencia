...
const isCPFValido = require('../helpers/verificacaoCPF')
...
router.post('/editar-perfil', async function (req, res) {
  ...
  // Verficação de CPF
  if (!isCPFValido(req.body.cpfIntegrante)) {
    req.flash('error_msg', 'CPF Inválido! Tente novamente.')
    res.redirect('/restrito/editar-perfil')
    retornar = true
  }
...
router.post('/registrar/integrante', eCoordenador, async function (req, res) {
  ...
  if (!isCPFValido(req.body.cpfIntegrante)) {
    req.flash('error_msg', 'CPF Inválido! Tente novamente.')
    res.redirect('/restrito/criar/integrantes')
    retornar = true
  }
...
router.post('/update/integrante/:id', eCoordenador, function (req, res) {
  ...
  if (!isCPFValido(req.body.cpfIntegrante)) {
    req.flash('error_msg', 'CPF Inválido! Tente novamente.')
    res.redirect('/restrito/editar/integrantes/' + req.params.id)
    retornar = true
  }
